window.injectSharedComponents = () => {
    ...
    <div id="cart-sidebar" class="fixed top-0 right-0 h-full w-full max-w-md bg-darker border-l border-white/10 z-[70] transform translate-x-full transition-transform duration-500 ease-in-out flex flex-col shadow-2xl">
        ...
        <div id="cart-footer" class="hidden shrink-0 p-8 border-t border-white/10 bg-dark shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-10">
            ...
            <button id="btn-checkout" class="w-full bg-emerald-500 hover:bg-emerald-400 text-black py-4 rounded-full font-bold text-lg transition-colors shadow-[0_0_20px_rgba(16,185,129,0.3)]">Procedi al Checkout</button>
        </div>
    </div>

    <!-- CHECKOUT MODAL -->
    <div id="checkout-modal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-md z-[80] overflow-y-auto">
        <div class="min-h-screen px-4 py-12 flex items-center justify-center text-center">
            <div class="inline-block w-full max-w-4xl text-left align-middle transition-all transform glass-panel border border-white/10 shadow-2xl rounded-3xl relative overflow-hidden bg-darker">
                <div class="p-8 border-b border-white/10 flex justify-between items-center bg-dark">
                    <div class="flex items-center gap-3">
                        <i data-lucide="lock" class="w-5 h-5 text-emerald-500"></i>
                        <h2 class="text-2xl font-bold">Checkout Sicuro</h2>
                    </div>
                    <button id="btn-close-checkout" class="text-gray-400 hover:text-white transition-colors"><i data-lucide="x" class="w-6 h-6"></i></button>
                </div>
                <div class="p-8 grid md:grid-cols-2 gap-12">
                    <div class="flex flex-col gap-6">
                        <h3 class="text-lg font-semibold uppercase tracking-widest text-gray-400 border-b border-white/10 pb-2">Dati di Spedizione</h3>
                        <div class="flex flex-col gap-4">
                            <input type="text" placeholder="Nome Completo" required maxlength="100" autocomplete="off" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 text-white placeholder-gray-600">
                            <input type="email" placeholder="Email per tracking" required maxlength="100" autocomplete="off" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 text-white placeholder-gray-600">
                            <input type="text" placeholder="Indirizzo e Numero Civico" required maxlength="100" autocomplete="off" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 text-white placeholder-gray-600">
                            <div class="grid grid-cols-2 gap-4">
                                <input type="text" placeholder="Città" required maxlength="100" autocomplete="off" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 text-white placeholder-gray-600">
                                <input type="text" placeholder="CAP" required maxlength="100" autocomplete="off" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 text-white placeholder-gray-600">
                            </div>
                        </div>

                        <h3 class="text-lg font-semibold uppercase tracking-widest text-gray-400 border-b border-white/10 pb-2 mt-4">Corriere</h3>
                        <select id="shipping-select" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 text-white text-sm appearance-none cursor-pointer">
                            <option value="0">BRT Standard (Gratis)</option>
                            <option value="6.90">GLS Express (24/48h) - €6.90</option>
                            <option value="12.90">DHL Premium (24h) - €12.90</option>
                        </select>
                    </div>
                    <div class="bg-white/5 p-6 rounded-2xl flex flex-col h-full border border-white/10">
                        <h3 class="text-lg font-semibold uppercase tracking-widest text-white border-b border-white/10 pb-2 mb-4">Riepilogo Ordine</h3>
                        <div class="flex justify-between items-center mb-4">
                            <span class="text-gray-400">Prodotti nel carrello <span id="checkout-qty">x1</span></span>
                            <span class="text-white font-medium" id="checkout-subtotal">€69.99</span>
                        </div>
                        <div class="flex justify-between items-center mb-4 text-sm border-b border-white/10 pb-4">
                            <span class="text-gray-400">Spedizione</span>
                            <span class="text-emerald-500 font-medium" id="checkout-shipping-cost">Gratuita</span>
                        </div>
                        <div class="mt-4 flex justify-between items-center">
                            <span class="font-bold text-lg text-white">Totale</span>
                            <span class="text-3xl font-bold text-white" id="checkout-total">€69.99</span>
                        </div>
                        <button class="w-full bg-emerald-500 hover:bg-emerald-400 text-black py-4 rounded-xl font-bold text-lg transition-colors mt-auto shadow-[0_0_20px_rgba(16,185,129,0.3)]">Conferma Ordine e Paga</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-12 md:gap-8 items-start border-b border-white/10 pb-12 mb-8">
            <div class="flex flex-col gap-4">
                ...
                <p class="text-gray-500 text-sm mt-2 leading-relaxed">
                    Ingegneria per le massime prestazioni. Precisione chirurgica e qualità senza compromessi.
                </p>
